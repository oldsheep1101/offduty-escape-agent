...
class EscapeRequest(BaseModel):
    """下班逃离请求"""
    origin: str = Field(..., description="起点地址", example="北京市朝阳区望京SOHO")
    destination: str = Field(..., description="终点地址", example="北京市海淀区五道口华联")
...
class MovieShowtime(BaseModel):
    """电影场次"""
    time: str = Field(..., description="开场时间 HH:MM", example="19:00")
    movie_name: str = Field(..., description="电影名称", example="速度与激情10")
    duration: int = Field(default=120, description="片长(分钟)")
    hall: Optional[str] = Field(default="", description="影厅")
    available_seats: int = Field(default=50, description="余座")
...
class Cinema(BaseModel):
    ...
    location: Location = Field(..., description="经纬度坐标")
    distance: float = Field(..., description="距离(米)")
    showtimes: List[MovieShowtime] = Field(default=[], description="可选场次")
    poi_id: Optional[str] = Field(default="", description="POI ID")
...
class RouteSegment(BaseModel):
    ...
    route_type: str = Field(..., description="路线类型: walking/driving")
...
class EscapePlan(BaseModel):
    ...
    selected_showtime: Optional[MovieShowtime] = Field(default=None, description="选择的场次")
    dinner_before_or_after: str = Field(default="after", description="电影前进食还是电影后进食: before/after")

    # 餐厅安排
    restaurant: Optional[Restaurant] = Field(default=None, description="餐厅信息")
    dinner_time: str = Field(default="", description="晚餐时间")

    # 时间线
    timeline: List[Dict[str, Any]] = Field(default=[], description="详细时间线")
    overall_suggestions: str = Field(default="", description="总体建议")


class EscapePlanResponse(BaseModel):
    """逃离计划响应"""
    success: bool = Field(..., description="是否成功")
    message: str = Field(default="", description="消息")
    data: Optional[EscapePlan] = Field(default=None, description="逃离计划数据")
